...
@section('page') Virtual POB @endsection
@section('content')
<div class="row">
                  ...
                  <div class="d-flex align-items-center">
                      <div class="flex-grow-1 overflow-hidden ms-0">
                                  ...
                                  @if ($box->aprooved == 1)
                                      <ul class="list-unstyled mb-0 vstack">
                                          ...
                                          <li><i class="ri-map-pin-line align-middle me-1 text-muted fs-16"></i>
                                             @if($box->pob_type == 'Individual')
                                              {{ $box->homeAddress == null ? 'Address not provided' : $box->homeAddress }}
                                             @else
                                             {{ $box->officeAddress == null ? 'Address not provided' : $box->officeAddress }}
                                             @endif
                                          </li>
                                          <li>
                                              <div class="text-center">
                                                  <a href="{{ route('customer.addressing.index',encrypt($box->id)) }}" class="text-muted text-decoration-underline">Show Address</a>
                                              </div>
                                          </li>
                                      </ul>
                                  @else
                          ...
                          <div class="d-flex justify-content-center align-items-center">
                              ...
                              @elseif($box->aprooved == 3)
                              {{-- reject --}}
                                <span class="badge bg-danger w-100">Application Rejected</span>
                              @else
                               @php $id = App\Models\Box::where('pob', $box->pob)->first()->id; @endphp
                              <a href="{{ route('customer.virtualPob.details', encrypt($id) ) }}" class="btn btn-primary btn-sm w-100"><span>P.O B Informations</span></a>
                              @endif
                            </div>
              ...
              <div class="modal-dialog modal-dialog-centered">
                  <div class="modal-content">
                      ...
                      <form class="tablelist-form" method="post"
                          action="{{ route('customer.virtualPob.update',$box->id) }}" enctype="multipart/form-data">
                          @csrf
                          @method('PUT')
        ...
        <div class="alert alert-danger alert-dismissible alert-additional fade show mb-xl-0" role="alert">
            <div class="alert-body">
                  <div class="d-flex">
                    ...
                    <div class="flex-grow-1">
                        <h5 class="alert-heading">No P.O BOX Application Found</h5>
