        ...
        <div class="card">
            ...
            <div class="card-body">
                <form class="tablelist-form" method="post"
                    action="{{ route('branch.sellingpostel.update', $outboxing->out_id) }}">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        <div class="col-6">
                            <input type="hidden" id="total_amount" name="total_amount"
                                value="{{ $outboxing->amount }}">
                            <label class="form-label" for="validationCustom01">Full Name</label>
                            <input name="name" type="text" value="{{ old('name', $outboxing->name) }}"
                                class="form-control" id="validationCustom01" autocomplete="off">
                        </div>
                        <div class="col-6">
                            <label class="form-label" for="validationCustom01">MOBILE PHONE </label>
                            <input name="phone" value="{{ old('phone', $outboxing->phone) }}" required type="text"
                            ...
                            <table id="itemsTable" class="table table-nowrap">
                                ...
                                <tbody>
                                    @foreach ($outboxing->products as $product)
                                        <tr data-id="{{ $product->item_id }}">
                                            ...
                                            <td> <strong>{{ $product->item->name }}</strong> <input type="hidden"
                                                    value="{{ $product->item_id }}" name="items[]" />
                                            ...
                                            <td><input type="number" name="amounts[]" value="{{ $product->amount }}"
                                ...
                                <tfoot>
                                    <tr>
                                        ...
                                        <td><span id="totalFooterAmount">{{ $outboxing->amount }}</span></td>
                                        <td></td>
                                    </tr>
                                </tfoot>
        ...
        $(document).ready(function() {
            ...
            $('#items').change(function() {
                ...
                $('#itemsTable tbody').append(`
                    <tr data-id="${itemId}">
                        <td>${rowCount}</td>
                        <td> <strong>${itemName}</strong> <input type="hidden" value="${itemId}" name="items[]"/></td><input type="hidden" value="${category}" name="category[]"/>
