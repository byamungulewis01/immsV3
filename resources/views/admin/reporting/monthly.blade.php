            ...
            <div class="card-body">
                <table id="datatable" class="table table-centered table-hover align-middle table-nowrap mb-0"
                    ...
                    <tbody>
                        ...
                        @php
                            $date = Carbon\Carbon::parse($item->pdate)->format('m-Y');
                        @endphp
                        <tr>
                            <td>{{ $loop->iteration }}</td>
                            <td>{{ Carbon\Carbon::parse($item->pdate)->format('F Y') }}</td>
                            <td>{{ number_format($item->total_amount,2) }}</td>
                            <td>
                                <a href="{{ route('admin.reporting.monthly_details') }}?date={{ $date }}"
                                    class="btn btn-sm btn-primary">View Detailed</a>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
