...
@section('body')
    ...
    @php
        $total = 0;
        $supplier_id = $purchase->first()->supplier_id;
    @endphp
    <div class="row justify-content-center">
        <div class="col-xxl-10">
            <div class="card">
                <form method="post" action="{{ route('admin.purchase.update') }}" class="needs-validation">
                    @method('PUT')
                    @csrf
                    <div class="card-body border-bottom border-bottom-dashed p-4">
                        <div class="row">
                            <div class="col-lg-6">
                                ...
                                <div>
                                    <div class="mb-3">
                                        <label for="companyAddress">UPDATE PURCHASE LIST FORM</label>
                                    </div>
                                        ...
                                        <select name="supplier_id" required class="form-select bg-light border-0">
                                            ...
                                            @foreach ($suppliers as $item)
                                                <option value="{{ $item->id }}"
                                                    @if (old('supplier_id',$supplier_id) == $item->id) selected @endif>
                            ...
                            <table id="itemsTable" class="table table-nowrap">
                                ...
                                <tbody>
                                    ...
                                    @endphp
                                        <tr data-id="{{ $item->item_id }}">
                                            <td>{{ $item->item_id }}</td>
                                            <td> <strong>{{ $item->item->name }}</strong>
                                                 <input type="hidden" value="{{ $item->item_id }}" name="item_id[]" />
                                                 <input type="hidden" value="{{ $item->purcha_id }}" name="purcha_id[]" />
                                                </td>
                                            <td><input type="number" min="1" name="qty[]" value="{{ $item->quantity }}"
                                                    class="form-control bg-light border-0 quantity" required> </td>
                                            <td><input type="number" name="price[]" value="{{ $item->price }}"
                                                    class="form-control bg-light border-0 price" required> </td>
                                            <td><input type="number" name="total[]"
                                                    class="form-control bg-light border-0 amount" value="{{ $item->total }}" readonly> </td>
                                ...
                                <tfoot>
                                    <tr>
                                        ...
                                        <td><span id="grandTotal">{{ number_format($total) }}</span></td>
                                        <td></td>
                                    </tr>
                                </tfoot>
