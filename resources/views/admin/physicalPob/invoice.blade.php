...
<div style="font-weight: bold; font-size: 12pt;" >BN: {{ $box->id }}</div>
<div style=" font-size: 12pt;" >Client:{{ $box->box->name }}</div> <br>
...
<tr>
<td align="center">{{ $box->box->pob }}</td>
<td align="center">{{ number_format($box->amount) }} Frw</td>
...
Service: POB Rent<br> <br>
<div> Done at Kigali on {{ $box->created_at->format('Y-m-d') }}</div>
<div>
    <!-- {{ \App\Models\User::where('branch', $box->box->branch_id)->first()->name }} -->
{{ auth()->user()->name }}
</div>
<hr>
{{-- pages --}}
