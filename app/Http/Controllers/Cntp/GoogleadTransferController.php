...
class GoogleadTransferController extends Controller
{
    public function index()
    {
        $branches = Branch::all();

        return view('admin.register.googleadtransfer', compact('branches'));
    }
    public function registertr($id){
       ...
       // $branches = Branch::all();
        ...
        $inboxings = DB::table('inboxings')
            ...
            ->select('inboxings.*', 'branches.name')->where('inboxings.instatus','0')->where('mailtype','GAD')->where('inboxings.instatus','0')->where('location',decrypt($id))->orderBy('inboxings.id', 'desc')->get();
        $bras = Branch::where('id',decrypt($id))->get();
        $results = Transfer::where('mailtype','GAD')->where('touserid',decrypt($id))->get();
        return view('admin.register.googleadtransferformreg',compact('id','count','inboxings','employees','results','bras'));
    }
    public function googletn(Request $request)
    {
        $formField = $request->validate([
            'mnumber' => 'required',
            'touserid' => 'required',
        ]);
    $totalpmt = Branch::where('id',$request->touserid)->sum('gadt');
    ...
    if($totalpmt<$mnumber){
     ...
     Branch::where('id',$request->touserid)->decrement('gadt',$request->mnumber);
   ...
   $trnsId = $transfer->id;
    ...
    // invoice
    public function invoicegooglead($id)
    {
        $inboxings = DB::select("SELECT * FROM inboxings WHERE id IN (SELECT inid FROM transferdatails WHERE trid = ?)", [(decrypt($id))]);
        $pdf = Pdf::loadView('admin.register.invoicegooglead', compact('inboxings','id'))
        ->setPaper('a4', 'portrait');
         return $pdf->stream('GoogleadjacentMailTransfer.pdf');
    }
}
