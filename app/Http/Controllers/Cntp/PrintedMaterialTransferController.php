...
class PrintedMaterialTransferController extends Controller
{
    public function index()
    {
        $branches = Branch::all();

        return view('admin.register.printedmaterialtransfer', compact('branches'));
    }
    public function registertr($id){
       ...
       // $branches = Branch::all();
        ...
        $inboxings = DB::table('inboxings')
            ...
            ->select('inboxings.*', 'branches.name')->where('inboxings.instatus','0')->where('mailtype','PRM')->where('inboxings.instatus','0')->where('location',decrypt($id))->orderBy('inboxings.id', 'desc')->get();
        $bras = Branch::where('id',decrypt($id))->get();
        $results = Transfer::where('mailtype','PRM')->where('touserid',decrypt($id))->get();
        return view('admin.register.printedmaterialtransferformreg',compact('id','count','inboxings','employees','results','bras'));
    }
    public function printedn(Request $request)
    {
        $formField = $request->validate([
            'mnumber' => 'required',
            'touserid' => 'required',
        ]);
    $totalpmt = Branch::where('id',$request->touserid)->sum('prmt');
    ...
    if($totalpmt<$mnumber){
     ...
     Branch::where('id',$request->touserid)->decrement('prmt',$request->mnumber);
   ...
   $trnsId = $transfer->id;
    ...
    // invoice
    public function invoiceprinted($id)
    {
        $inboxings = DB::select("SELECT * FROM inboxings WHERE id IN (SELECT inid FROM transferdatails WHERE trid = ?)", [(decrypt($id))]);
        $pdf = Pdf::loadView('admin.register.invoiceprintedmaterial', compact('inboxings','id'))
        ->setPaper('a4', 'portrait');
         return $pdf->stream('PrintedMailTransfer.pdf');
    }
}
