...
class OrdinaryLetterTransferController extends Controller
{
    public function index()
    {
        $branches = Branch::all();

        return view('admin.register.ordinarylettertransfer', compact('branches'));
    }
    public function registertr($id){
       ...
       // $branches = Branch::all();
        ...
        $inboxings = Inboxing::where('mailtype','ol')
        ->where('mailtranserf','0')
        ->where('instatus','0')
        ->where('location',decrypt($id))->get();
        $bras = Branch::where('id',decrypt($id))->get();
        $results = Transfer::where('mailtype','ol')->where('touserid',decrypt($id))->get();
        return view('admin.register.ordinarylettertransferformreg',compact('id','count','inboxings','employees','results','bras'));
    }
    public function ordletterapi($id,$user)
    {
   ...
   $trnsId = $transfer->id;
    ...
    public function storeletterod(Request $request)
    {
        $formField = $request->validate([
            'mnumber' => 'required',
            'touserid' => 'required',
        ]);
    $totalpmt = Branch::where('id',$request->touserid)->sum('olt');
    ...
    if($totalpmt<$mnumber){
     ...
     Branch::where('id',$request->touserid)->decrement('olt',$request->mnumber);
    ...
    public function invoiceod($idd)
    {
        ...
        $pdf = Pdf::loadView('admin.register.invoiceordinaryletter', compact('inboxings','idd','number'))
        ->setPaper('a4', 'portrait');
         return $pdf->stream('Ordinary LetterTransfer.pdf');
    }
}
