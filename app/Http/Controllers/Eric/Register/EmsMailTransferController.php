...
class EmsMailTransferController extends Controller
{
    public function index()
    {
        $branches = Branch::all();

        return view('admin.register.emsmailtransfer', compact('branches'));
    }
    public function registertr($id){
       ...
       // $branches = Branch::all();
        ...
        $inboxings = Inboxing::where('mailtype','ems')
        ->where('mailtranserf','0')
        ->where('instatus','0')
        ->where('location',decrypt($id))->get();
        $bras = Branch::where('id',decrypt($id))->get();
        $results = Transfer::where('mailtype','ems')->where('touserid',decrypt($id))->get();
        return view('admin.register.emsmailtransferformreg',compact('id','count','inboxings','employees','results','bras'));

    }
    public function emApi($id,$user)
    {
   ...
   $trnsId = $transfer->id;
    ...
    $totalpmt = Branch::where('id',$request->touserid)->sum('emst');
    ...
    if($totalpmt<$mnumber){
     ...
     Branch::where('id',$request->touserid)->decrement('emst',$request->mnumber);
    ...
    public function invoicereg($idd)
    {
        ...
        $pdf = Pdf::loadView('admin.register.invoiceems', compact('inboxings','idd','number'))
        ->setPaper('a4', 'portrait');
         return $pdf->stream('EMS MailTransfer.pdf');
    }
}
