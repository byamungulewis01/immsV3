...
class RegisteredMailPaymentController extends Controller
{
    public function index()
    {
        $inboxings = Inboxing::where('location', auth()->user()->branch)->orderBy('id', 'desc')
            ->where('mailtype', 'r')->where('paystatus', '0')->where('instatus', '3')
            ->limit(1000)->get();
        return view('admin.backoffice.rmailpay', compact('inboxings'));
    }
